<?php
	...
	//if(isset($_POST['uid']) && $_POST['uid'] == $user->uid && isset($_POST['record_date'])){
...
$bedTime = "昨天 晚上 10:00";
	if(isset($bedTime)){
		...
		if($Day=="昨天"){
			...
			//$bedTimeDate = date( "Y-m-d", strtotime($day)-86400);
			//$bedTimeDate = date( "Y-m-d", strtotime($time)-86400);
	...
	$query = sprintf("INSERT INTO jhs_hss.sleepinfo_basic_sleep_pattern(uid,rid,recordTime,bedTime,asleepTime,awakeTimes,wakeBy,exceptedTime,wakeTime,riseTime,morningFeel,dayFeel,nightMood,sleepQuality,device) VALUES ( %d,'%s','%s','%s','%s',%d,'%s','%s','%s','%s',%d,%d,%d,%d,'%s')",$user->uid,$rid,$date,$bedTime,$asleepTime,$awakeTimes,$wakeType,$exceptedTime,$wakeupTime,$riseTime,$morningFeel,$dayFeel,$nightMood,$sleepQuality,SleepDiary);
	//db_query($query);
	echo $query;
	}
else{
